...
class RBAC {
    ...
    static public function IsAccess($authId,$module) {
        // Db方式权限数据
        $db     =   Db::getInstance(C('RBAC_DB_DSN'));
        $sql    =   "select count(*) from ".
                ...
                $table['access']." as access ,".
                $table['node']." as node ".
                "where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=2 and node.name='{$module}' and node.status=1";
        if(count($db->query($sql)))
            return false;
        return true;
    }
}